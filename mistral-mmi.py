...
def train_simple_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid):
    ...
    optimizer = optim.Adam([
        {'params': trainable_params, 'lr': 0.00005},
...
def process(config, task, shot):
    ...
    file_handler = logging.FileHandler(f'SCKD-mistral-mmi-logs-{config.task}.log')
    ...
    for rou in range(1):
        ...
        encoder = LlamaLMClassification.from_pretrained("mistralai/Mistral-7B-v0.3",
#                                                   pad_token_id=32004,
