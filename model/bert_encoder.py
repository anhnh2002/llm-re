...
class PrefixLlamaModel(LlamaPreTrainedModel):
    """
    ...
    def forward(
        ...
        if prefix_hidden_states is not None:
            batch_size, seq_length, hidden_length = inputs_embeds.size()
            prefix_hidden_states = prefix_hidden_states.squeeze().reshape(batch_size, -1, 2, hidden_length) # B, layers, 2, hidden_length
        ...
        for i, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if prefix_hidden_states is not None:
                    ...
                    hidden_states = torch.cat([prefix_hidden_states[:,i,:,:].to(device), hidden_states[:, 2:, :]], dim=1)
...
class LlamaClassification(LlamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        # self.num_labels = config.num_labels
        self.model = PrefixLlamaModel(config)
        # self.ln = nn.Linear(config.hidden_size, self.config.hidden_size, bias=True)
        # self.dropout = nn.Dropout(0.1)
