...
from model.bert_encoder import Bert_Encoder, GemmaClassification
...
device_map = {'model.embed_tokens': int(config.device0.index),
 ...
 'model.layers.11': int(config.device1.index),
 'model.layers.12': int(config.device1.index),
 'model.layers.13': int(config.device1.index),
 'model.layers.14': int(config.device1.index),
 'model.layers.15': int(config.device1.index),
 'model.layers.16': int(config.device1.index),
...
def train_mem_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid, new_relation_data,
    ...
    if prev_encoder is not None:
        for labels, _, tokens, att_mask, id in tqdm(data_loader):
            att_mask = torch.stack([x.to(config.device0) for x in att_mask],dim=0)
            tokens = torch.stack([x.to(config.device0) for x in tokens], dim=0)
            _reps = encoder(tokens, attention_mask=att_mask)
...
if __name__ == '__main__':
    ...
    for rou in range(config.total_round):
        ...
        encoder = GemmaClassification.from_pretrained("google/gemma-2b",
#                                                   pad_token_id=32004,
