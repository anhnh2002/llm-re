...
device_map = {'model.embed_tokens': 0,
 'model.layers.0': int(config.device0[-1]),
 'model.layers.1': int(config.device0[-1]),
 'model.layers.2': int(config.device0[-1]),
 'model.layers.3': int(config.device0[-1]),
 'model.layers.4': int(config.device0[-1]),
 'model.layers.5': int(config.device0[-1]),
 'model.layers.6': int(config.device0[-1]),
 'model.layers.7': int(config.device0[-1]),
 'model.layers.8': int(config.device0[-1]),
 'model.layers.9': int(config.device0[-1]),
 'model.layers.10': int(config.device0[-1]),
 'model.layers.11': int(config.device0[-1]),
 'model.layers.12': int(config.device0[-1]),
 'model.layers.13': int(config.device0[-1]),
 'model.layers.14': int(config.device0[-1]),
 'model.layers.15': int(config.device0[-1]),
 'model.layers.16': int(config.device0[-1]),
 'model.layers.17': int(config.device1[-1]),
 'model.layers.18': int(config.device1[-1]),
 'model.layers.19': int(config.device1[-1]),
 'model.layers.20': int(config.device1[-1]),
 'model.layers.21': int(config.device1[-1]),
 'model.layers.22': int(config.device1[-1]),
 'model.layers.23': int(config.device1[-1]),
 'model.layers.24': int(config.device1[-1]),
 'model.layers.25': int(config.device1[-1]),
 'model.layers.26': int(config.device1[-1]),
 'model.layers.27': int(config.device1[-1]),
 'model.layers.28': int(config.device1[-1]),
 'model.layers.29': int(config.device1[-1]),
 'model.layers.30': int(config.device1[-1]),
 'model.layers.31': int(config.device1[-1]),
 'model.norm': int(config.device1[-1]),
 'lm_head': int(config.device1[-1]),
 'info_nce_fc': int(config.device1[-1])}
...
def train_mem_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid, new_relation_data,
    ...
    for epoch_i in range(epochs):
        ...
        for step, (labels, _, tokens, att_mask, id) in tqdm(enumerate(data_loader)):
            ...
            loss = loss + infoNCE_loss.to(loss.device)*config.infonce_lossfactor
...
# def prep_eval(config, encoder, test_data, map_relid2tempid):
...
#         tokens = torch.stack([x.to(config.device0) for x in tokens],dim=0)
#         reps = encoder(tokens)
#         result.append(reps[0].cpu().numpy())
        

# def prev_eval(config, pre_reps, dropout_layer, classifier, test_data, seen_relations, map_relid2tempid):
#     data_loader = get_data_loader(config, test_data, batch_size=1, shuffle=False)
#     # encoder.eval()
#     dropout_layer.eval()
#     classifier.eval()
...
#         # tokens = torch.stack([x.to(config.device0) for x in tokens],dim=0)
#         # reps = encoder(tokens)
#         reps = torch.Tensor(pre_reps[step]).unsqueeze(0).to(config.device1)
#         reps, _ = dropout_layer(reps)
#         logits = classifier(reps)
...
def data_augmentation(config, encoder, train_data, prev_train_data):
    ...
    for step, batch_data in tqdm(enumerate(data_loader)):
        ...
        tokens = torch.stack([x.to(config.device0) for x in tokens],dim=0)
        with torch.no_grad():
            feature, mask_output = encoder(tokens, attention_mask=att_mask, get_feature=True)
            # print(feature.shape)
