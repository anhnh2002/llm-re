...
from peft import PrefixEncoder, PrefixTuningConfig#, prepare_model_for_int8_training
...
def train_simple_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid):
    ...
    encoder, prefix_encoder = encoder
    ...
    optimizer = optim.Adam([
        {'params': prefix_encoder.parameters(), 'lr': 0.00005},
    ...
    prefix_encoder = accelerator.prepare_model(prefix_encoder)
    ...
    for epoch_i in range(epochs):
        ...
        for step, batch_data in tqdm(enumerate(data_loader)):
            ...
            prefix_hidden_states = prefix_encoder(torch.tensor([0]*labels.shape[0]).to(config.device0))

            reps = encoder(tokens, attention_mask=att_mask, prefix_hidden_states=prefix_hidden_states)
...
def train_mem_model(config, encoder, dropout_layer, classifier, training_data, epochs, map_relid2tempid, new_relation_data,
    ...
    if prev_encoder is not None:
        for labels, _, tokens, att_mask, id in tqdm(data_loader):
            att_mask = torch.stack([x.to(config.device0) for x in att_mask],dim=0)
            tokens = torch.stack([x.to(config.device0) for x in tokens], dim=0)
            prefix_hidden_states = prefix_encoder(torch.tensor([0]*tokens.shape[0]).to(config.device0))
            _reps = encoder(tokens, attention_mask=att_mask, prefix_hidden_states=prefix_hidden_states)
...
def select_data(config, encoder, dropout_layer, relation_dataset):
    ...
    for step, batch_data in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            prefix_hidden_states = prefix_encoder(torch.tensor([0]*tokens.shape[0]).to(config.device0))
            feature = dropout_layer(encoder(tokens, attention_mask=att_mask, prefix_hidden_states=prefix_hidden_states))[1].cpu()
...
def get_proto(config, encoder, dropout_layer, relation_dataset):
    ...
    for step, batch_data in tqdm(enumerate(data_loader)):
        ...
        with torch.no_grad():
            prefix_hidden_states = prefix_encoder(torch.tensor([0]*tokens.shape[0]).to(config.device0))
            feature = dropout_layer(encoder(tokens, attention_mask=att_mask, prefix_hidden_states=prefix_hidden_states))[1]
        features.append(feature)
